#!/usr/bin/env python3
"""Generate the Tencent Sports MITM rule from immutable local upstream files."""
...
def require_supported_upstream(upstream: Path) -> None:
    missing = [filename for filename in UPSTREAM_FILES if not (upstream / filename).is_file()]
    if missing:
        raise UpdateError("missing upstream file(s): " + ", ".join(missing))
...
def build_amrs(
    ...
    lines = [
        ...
        "# SOURCE-SHA256: "
        + "; ".join(f"{filename}={source_hashes[filename]}" for filename in UPSTREAM_FILES),
...
def generate_candidate(
    ...
    candidate = build_amrs(preserved_date, source_revision, source_hashes)
    if target.exists() and candidate == target.read_text(encoding="utf-8"):
        return candidate
    return build_amrs(dt.date.today().isoformat(), source_revision, source_hashes)
...
def main() -> int:
    ...
    if not re.fullmatch(r"[0-9a-f]{40}", args.source_revision):
        print("error: --source-revision must be a full lowercase Git SHA", file=sys.stderr)
        return 1
    source_dir = args.source_dir.resolve()
    target = repo_root() / TARGET_RELATIVE

    try:
        require_supported_upstream(source_dir)
        source_hashes = {
            filename: file_sha256(source_dir / filename)
            for filename in UPSTREAM_FILES
        }
        candidate = generate_candidate(target, args.source_revision, source_hashes)
        validate_amrs(candidate)
        validate_against_upstream(candidate, source_dir)
        update_target(target, candidate, check_only=args.check_only)
